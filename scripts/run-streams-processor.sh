...
KAFKA_STREAMS_PROCESSOR_JAR="jars/kafka-streams-processor.jar"
...
jar_file1=$KAFKA_STREAMS_PROCESSOR_JAR
...
function run_jar_with_agent() {
    ...
    if [ -e "$main_jar" ]; then
        ...
        if [ -e "$agent_jar" ]; then
            ...
            java -javaagent:"$agent_jar" -Dotel.metrics.exporter=prometheus -Dotel.exporter.prometheus.port=9465 -jar "$main_jar"
        else
