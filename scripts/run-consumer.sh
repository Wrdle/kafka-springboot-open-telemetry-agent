...
KAFKA_CONSUMER_JAR="jars/kafka-consumer.jar"
...
function run_jar_with_agent() {
    ...
    if [ -e "$main_jar" ]; then
        ...
        if [ -e "$agent_jar" ]; then
            ...
            java -javaagent:"$agent_jar" -Dotel.metrics.exporter=prometheus -Dotel.exporter.prometheus.port=9466 -jar "$main_jar"
        else
